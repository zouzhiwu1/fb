# football-betting-miniprogram

微信小程序客户端，与 **football-betting-mobile** 使用同一套 **football-betting-platform** API（登录、注册、账户、会员、充值、曲线图查询）。
...
2. **API 地址**  
   ...
   须与 **football-betting-mobile** 的 `EXPO_PUBLIC_API_BASE_URL` 指向同一套后端。
...
2. 选择「导入项目」，目录选本仓库下的 `football-betting-miniprogram`。
3. 填写 AppID 后编译预览。
