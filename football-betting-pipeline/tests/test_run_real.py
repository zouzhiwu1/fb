...
from run_real import _compute_default_time_window, _setup_logging, main


def test_setup_logging_returns_logger(monkeypatch, tmp_path):
    monkeypatch.setattr("run_real.DEBUG_LOG_DIR", str(tmp_path))
    log = _setup_logging()
    assert log is not None
    assert log.name == "run_real"


def test_compute_default_time_window_before_cutoff(monkeypatch):
    # 固定临界点，避免依赖环境变量
    monkeypatch.setattr("run_real.CUTOFF_HOUR", 12, raising=False)
...
def test_main_no_args_uses_default_window_and_runs_steps(monkeypatch, tmp_path):
    monkeypatch.setattr("run_real.DEBUG_LOG_DIR", str(tmp_path))
    monkeypatch.setattr("run_real.LOG_RETENTION_DAYS", 7)
    monkeypatch.setattr("sys.argv", ["run_real.py"])
    run_calls = []
...
def test_main_two_valid_args_parses_and_runs_steps(monkeypatch, tmp_path):
    monkeypatch.setattr("run_real.DEBUG_LOG_DIR", str(tmp_path))
    monkeypatch.setattr("sys.argv", ["run_real.py", "2026030812", "2026030911"])
    run_calls = []
...
def test_main_invalid_args_exits(monkeypatch, tmp_path):
    monkeypatch.setattr("run_real.DEBUG_LOG_DIR", str(tmp_path))
    monkeypatch.setattr("sys.argv", ["run_real.py", "one", "two"])
...
def test_main_start_after_end_exits(monkeypatch, tmp_path):
    monkeypatch.setattr("run_real.DEBUG_LOG_DIR", str(tmp_path))
    monkeypatch.setattr("sys.argv", ["run_real.py", "2026030911", "2026030812"])
    with pytest.raises(SystemExit):
        main()
...
def test_main_bad_time_format_exits(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("run_real.DEBUG_LOG_DIR", str(tmp_path))
    monkeypatch.setattr("sys.argv", ["run_real.py", "2026023012", "2026023013"])  # 无效日期
    with pytest.raises(SystemExit):
        main()
...
def test_main_subprocess_failure_exits(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("sys.argv", ["run_real.py", "2026030812", "2026030911"])
    from unittest.mock import MagicMock
    monkeypatch.setattr("run_real.subprocess.run", lambda *a, **k: MagicMock(returncode=1))
    with pytest.raises(SystemExit):
        main()
