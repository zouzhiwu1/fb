...
**功能**：打开智云比分页，进入「足球」→「即时比分」→「足彩」→「北单」，等待表格刷新后，仅下载状态为空的比赛（状态列为空白或「-」），逐场点击导出并下载对应的 `.xls` 文件。文件按配置的下载目录与跨天临界点保存到子目录 `{YYYYMMDD}/`，文件名含主客队与时间点（如 `主队 VS 客队2026030807.xls`）。
...
| `CRAWLER_TRIGGER_HOURS` | 定时任务触发整点（逗号分隔，0～23），仅影响 `main.py` 无参数时的区间计算 | `2,4,6,13,15,17,19,21,23` |
