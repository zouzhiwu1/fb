...
DATABASE_URL = os.environ.get(
    "DATABASE_URL",
    "mysql+pymysql://root:123456@localhost:3306/fb",
)
...
ensure_mysql_user_not_placeholder(
    ...
    error_message=(
        ...
        "将 DATABASE_URL 改为例如：mysql+pymysql://root:你的密码@localhost:3306/fb\n"
...
# 优先用环境变量；否则尝试「项目根/fb-report」（platform 在项目内时）
_def_report = os.path.join(WORK_SPACE, "fb-report")
_config_dir = os.path.dirname(os.path.abspath(__file__))
_repo_report = os.path.join(os.path.dirname(_config_dir), "fb-report")
...
# 平台日志目录（与 pipeline 一致：仓库根下 fb-log）
