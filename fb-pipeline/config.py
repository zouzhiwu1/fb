...
爬虫配置。可通过环境变量覆盖（若存在 .env 会先加载）：
  WORK_SPACE  工作目录根路径，其下放置 fb-data、fb-report、fb-log 等
...
# 始终先加载与 config.py 同目录下的 .env（即 fb-pipeline/.env），
...
# 默认使用当前文件所在目录的上一级（即包含 fb-data / fb-log / fb-report 的目录）。
...
_DEFAULT_TARGET_LEAGUES = (
    "澳澳超,挪超,挪甲,丹超,丹甲,波兰甲,波兰超,波兰杯,葡甲,葡超,瑞士甲,瑞士超,南美杯,美冠杯,"
    "英甲,英冠,英乙,英超,足总杯,联赛杯,德甲,德乙,德国杯,意甲,意乙,意大利杯,意杯,法甲,法乙,"
    ...
    "比甲,比乙,荷甲,荷乙,捷甲,捷甲冠,冰岛超,瑞典超,国际友谊,世界杯,瑞典甲"
)
