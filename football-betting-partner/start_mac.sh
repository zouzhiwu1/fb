#!/usr/bin/env bash
# macOS：后台启动 partner 进程（nohup）。业务日志：<仓库根>/football-betting-log/partner_YYYYMMDD.log
# nohup 合并输出：<仓库根>/football-betting-log/partner_nohup.log
# 停止：./stop_mac.sh
set -euo pipefail
...
DAY="$(date +%Y%m%d)"
PARTNER_LOG="${LOG_DIR}/partner_${DAY}.log"
...
echo $! >"$PID_FILE"
echo "已在后台启动 football-betting-partner，PID=$(cat "$PID_FILE") PORT=$PORT"
